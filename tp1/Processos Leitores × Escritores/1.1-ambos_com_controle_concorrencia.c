...
int main() {
    ...
    int leituras_por_thread = num_vendas/2; 
    ...
    for (int i = 0; i < (num_consultas + num_vendas); i++) { // só um laço para que eles conseguirem se intercalar no mutex
        int j = 0;
        int k = 0;
        if (j < num_consultas) {
        ...
        if (k < num_vendas) {
            ...
            pthread_create(&threads[i], NULL, realizar_venda, (void *)args);
        }
    }
