...
void *produzir(void *arg) {
    ...
    for (int i = 0; i < total_a_produzir; i++) {
        ...
        printf("[PRODUTOR %d]: Aguardando espaço (empty)...\n", id);
        sem_wait(&empty); 
        ...
        printf("[PRODUTOR %d]: ENTROU na RC. Inserindo item %d na pos %d\n", id, item_produzido, in);
...
void *consumir(void *arg) {
    ...
    for (int i = 0; i < total_a_consumir; i++) {
        printf("[CONSUMIDOR %d]: Aguardando item (full)...\n", id);
        sem_wait(&full); 
...
int main() {
    ...
    printf("--- Simulacao Produtores - 1 Consumidor ---\n");
